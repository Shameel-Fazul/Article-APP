...
const rootReducer = (state = initState, action) => {
    if (action.type === 'DELETE_POST') {
        let newPosts = state.posts.filter(post => { //filter method filters out if it doesn't meet the conditons.
            return action.id !== post.id
        });
        return {
            ...state,      //takes all the states using spread operators, so it doesn't overwrite.
            posts: newPosts
        }
    }
    return state;
